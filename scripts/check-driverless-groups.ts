 ...
 *
 * Two shapes are reported separately because they are not the same problem:
 *
...
/**
 * Splits groups into the two problem shapes, leaving healthy groups out.
 *
 * A group with more than one DRIVER is *not* reported: it is a different
 * invariant (see SCRUM-291), and every management path still works, so folding
 * it in here would bury the groups that are actually stuck.
 */
export const findDriverlessGroups = (
...
const main = async () => {
  ...
  try {
    ...
    if (driverless.length === 0 && empty.length === 0) {
      console.log("\n✓ every carpool group has a driver.");
      return;
    }
