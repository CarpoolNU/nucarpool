...
- **`#__next` owns the app's height; a page root must not re-declare it.** [`globals.css`](src/styles/globals.css) sets it to `100dvh` so the app tracks the mobile viewport as browser chrome comes and goes, and a page root inherits that with `h-full`. Tailwind's `*-screen` height utilities compile to `vh`, which on mobile is the _large_ viewport height, so one of them on a page root opts back out of that fix one element inside it — the defect behind SCRUM-433 and SCRUM-453. Those utilities are deliberately **not** named here: per the bullet above, naming one would ship it as CSS. Grep `-screen` under `src/` instead. A `vh` **max**-height on an inner scroll container is not a page shell and is fine.
