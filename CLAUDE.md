...
- `yarn build:preview` **no longer exists** — it was deleted in SCRUM-249. It ran `prisma db push` auto-confirmed with `echo "y"` and then re-seeded, so it could force-alter a schema and wipe data. It mattered because [`amplify.yml`](amplify.yml) ends in `yarn run build:${BUILD_ENV}` with `BUILD_ENV` set in the Amplify console, which made `preview` a reachable value. The deny rules in [`.claude/settings.json`](.claude/settings.json) are kept anyway, so they would still fire if it were reintroduced. Use `yarn build` to test a build.
