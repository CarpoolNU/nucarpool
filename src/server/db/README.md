...
| `request.acceptanceNotificationPendingSince` | `markRequestAccepted`, when it flips `status` to `ACCEPTED` | `sendAcceptanceNotification` |
...
- **All three default to "nothing owed"**, so every row written before its column existed can never send. The opening message `requests.create` writes is left unmarked, because the request email announces it.
